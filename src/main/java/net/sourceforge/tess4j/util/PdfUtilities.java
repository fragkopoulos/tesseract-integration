...
public class PdfUtilities {
    ...
    public static File convertPdf2Tiff(File inputPdfFile) throws IOException {
        ...
        try {
            ...
            if (pngFiles != null) {
                File _tempFileParent = null;
                for (File tempFile : pngFiles) {
                    if (_tempFileParent == null) {
                        _tempFileParent = tempFile.getParentFile();
                    }
                    // delete temporary PNG images
                    tempFile.delete();
                }
                // delete temporary PNG parent folder
                _tempFileParent.delete();
            }
        }
    }
    ...
    public static File[] convertPdf2Png(File inputPdfFile) {
        ...
        if (imageDir != null) {
            userDir = imageDir.getPath();
        }

        imageDir = new File(userDir + File.separator + random);
