...
//        boxes = TessAPI1.TessBaseAPIGetRegions(handle, pixa); // equivalent to TessPageIteratorLevel.RIL_BLOCK
    ...
    public void testTessBaseAPIProcessPages() {
        logger.info("TessBaseAPIProcessPages");
        File imageFile = new File(testResourcesDataPath, "eurotext.tif");
        String retry_config = null;
        int timeout_millisec = 0;
        String outputbase = "src/test/resources/test-results/eurotext1";
        // make sure parent dir exists
        File outputdir = new File(outputbase).getParentFile();
        if (!outputdir.exists()) {
            outputdir.mkdirs();
        }
        ...
        int rc = TessAPI1.TessBaseAPIProcessPages(handle, imageFile.getPath(), retry_config, timeout_millisec, renderer);
        assertEquals(TessAPI1.TRUE, rc);
    }

    /**
     * Test of TessBaseAPIProcessPage method, of class TessAPI1.
     */
    @Test
    public void testTessBaseAPIProcessPage() {
        logger.info("TessBaseAPIProcessPage");
        File imageFile = new File(testResourcesDataPath, "eurotext.tif");
        Leptonica leptInstance = Leptonica.INSTANCE;
        ...
        TessAPI1.TessBaseAPIInit3(handle, datapath, language);
        TessResultRenderer renderer = TessAPI1.TessTextRendererCreate(outputbase);
        int rc = TessAPI1.TessBaseAPIProcessPage(handle, pix, 0, imageFile.getName(), retry_config, timeout_millisec, renderer);
        assertEquals(TessAPI1.TRUE, rc);
    }
    ...
    public void testResultRenderer() throws Exception {
        ...
        String outputbase = "src/test/resources/test-results/outputbase1";
